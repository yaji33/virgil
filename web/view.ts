...
export function activity(events: Activity[]): string {
  return events.length
    ...
    <time datetime="${event.time}">${new Date(event.time).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</time></li>`,
        ...
        .join("")}</ol>`
    : `<div class="quiet-empty"><span class="empty-icon">↗</span><h3>A clear record, from the first step.</h3><p>Create a plan to start your session activity.</p></div>`;
}
  ...
  const heading =
    page === "overview"
      ...
      : page === "plans"
        ...
        : ["EVERY STEP, ACCOUNTED FOR", "Every step, accounted for.", "The decisions and changes made during this session."];
  return `<a class="skip-link" href="#main">Skip to workspace</a><div class="shell">
    <aside class="sidebar"><a class="brand" href="#" aria-label="Virgil overview"><span class="brand-mark">V</span>virgil<span class="brand-period">.</span></a><span class="workspace-label">PERSONAL WORKSPACE</span>
      <nav aria-label="Main navigation">${(
        ...
        .join("")}</nav>
      <div class="sidebar-note"><span class="small-tag">PROTOTYPE</span><p>A space to explore layout and decisions.<br>No exchange connected.</p><span class="fine">Plans last for this browser session. Refreshing clears them.</span></div><div class="profile"><span class="avatar">Y</span><div>You<span>Personal workspace</span></div><span class="profile-dot"></span></div></aside>
    ...
    ${page !== "activity" ? `<section class="scenario"><div><span class="scenario-icon" aria-hidden="true">◇</span><div><strong>Explore a capital conflict</strong><p>See what changes when another strategy needs the same funds.</p></div></div><button class="switch ${workspace.conflict ? "on" : ""}" data-action="scenario" role="switch" aria-checked="${workspace.conflict}" aria-label="Simulate capital conflict"><span></span></button></section>${plans(workspace)}` : `<section class="panel activity-panel"><div class="section-heading"><h2>Session activity</h2><span class="small-tag">LOCAL RECORD</span></div>${activity(workspace.activity)}</section>`}
    ${page === "overview" && workspace.activity.length ? `<section class="panel recent-panel"><div class="section-heading"><h2>Recent in this session</h2><span class="small-tag">LOCAL RECORD</span></div>${activity(workspace.activity.slice(0, 3))}</section>` : ""}
